...
const fs = require("fs");
const cardFile = require("../json/ticketsReportsCartes.json");
const idfile = require('../0-jsons/monID.json');

module.exports.run = async (bot, message, args) => {
  let taille = cardFile[-1].number;
  ...
  if(message.author.id != idfile.id && nomPlaintif != cardFile[id].plaintif){
  ...
  if(cardFile[id].nerf && message.author.id != idfile.id) return message.reply("Cette requête a déjà été traitée.")
  cardFile[id].nerf = validRequest;
  fs.writeFile("./json/ticketsReportsCartes.json", JSON.stringify(cardFile), (err) =>{
