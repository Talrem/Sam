  ...
  if(args[0]){
    ...
    for(j = 0; j < taille ; j+=10){
      for(i = 0; i < 10 && i+j < taille; i++){
        if(cardFile[i+j].nerf && cardFile[i+j].joueur == args[0]){
          messageToSend += "id : " + (i+j) + " - " + cardFile[i+j].carte
          switch (cardFile[i+j].nerf) {
            case 1:
    ...
    for(j = 0; j < taille ; j+=10){
      for(i = 0; i < 10 && i+j < taille; i++){
        if(cardFile[i+j].nerf){
          messageToSend += "id : " + (i+j) + " - " + cardFile[i+j].carte
          messageToSend += " `de " + cardFile[i+j].joueur + "`";
          switch (cardFile[i+j].nerf) {
            case 1:
              messageToSend += " (nerf de l'effet)";
              break;
