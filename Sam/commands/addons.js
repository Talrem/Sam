...
module.exports.run = async (bot, message, args) => {
  ...
  if(args.length){
    ...
    for(i = 0; i < taille ; i++){
      msg = "";
      msg += addonFile[i].name + " - " + baselink + addonFile[i].url + "\n";
      switch (args[0].toLowerCase()) {
        case "skin":
          if(addonFile[i].type == "skin"){
            skinToSend += msg;
            compteur++;
          }
          if(compteur==7){
        ...
        case "mode":
          ...
          break;
        case "texture":
          if(addonFile[i].type == "texture"){
            textureToSend += addonFile[i].name + " - " + addonFile[i].url + "\n";
            compteur++;
          }
          if(!compteur==7){
