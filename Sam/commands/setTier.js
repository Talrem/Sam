...
module.exports.run = async (bot, message, args) => {
  ...
  if(deckFile[args[0]].t == "Ban" && message.author.id != idfile.id || deckFile[args[0]].s == "1" && message.author.id != idfile.id) return message.reply("Un deck ban ou suspect ne peut être modifié par cette commande.")
  ...
  if(deckFile[args[0]])
    if(deckFile[args[0]].id != idJoueur && message.author.id != idfile.id) return message.reply("Le deck spécifié ne vous appartient pas.")
    deckFile[args[0]] = {
      id:idJoueur,
      u:username,
      n:nomDeck,
