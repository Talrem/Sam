...
module.exports.run = async (bot, message, args) => {
  ...
  for(i = 0; i < taille ; i++){
    ...
    if(requete == type[0]){
      if(cardFile[i].plaintif == nom || nom == "all")
        if(!cardFile[i].nerf) compteur++;
    }
    /*Combien pour l'utilisateur n'ont pas été traitées*/
    if(requete == type[3]){
      if(cardFile[i].joueur == nom || nom == "all")
        if(!cardFile[i].nerf) compteur++;
    }
    ...
    if(requete == type[1]){
      if(cardFile[i].plaintif == nom || nom == "all"){
        if(!cardFile[i].nerf){
          messageToSend += "id : " + i + " - " + cardFile[i].carte
          messageToSend += " `de " + cardFile[i].joueur + "`";
          messageToSend += "\n";
          compteur++;
        }
      }
