...
module.exports.run = async (bot, message, args) => {
  ...
  var j2 = 0;
  for(i = 0; i < taille; i++){
    switch (deckFile[i].id) {
      case idJoueur1:
        if(args.length == 2){
          if(deckFile[i].t.toLowerCase() == args[1].toLowerCase()){
            if(!(deckFile[i].t == "Ban" && args[1].toLowerCase() != "ban")){
              lesDeck1[j1] = deckFile[i];
              j1++;
            }
          }
        }else{
