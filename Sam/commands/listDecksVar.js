...
module.exports.run = async (bot, message, args) => {
  ...
  for(j=1; j < args.length;j++){
    for(i = 0; i < taille; i++){
      if(args[0].toLowerCase() == "self" && deckFile[i].id == message.author.id || args[0].toLowerCase() == "all"){
        if(deckFile[i].t.toLowerCase() == args[j].toLowerCase() || deckFile[i].n.toLowerCase() == args[j].toLowerCase() || deckFile[i].w.toLowerCase() == args[j].toLowerCase() || deckFile[i].c.toLowerCase() == args[j].toLowerCase() || deckFile[i].p.toLowerCase() == args[j].toLowerCase()){
          if(!isIn(lesID,i)){
